...
class BaseRepository(Generic[Model], ABC):
    ...
    async def get_one(
        ...
        return await self.session.get_one(self.model_type, ident, options=options)
    ...
    async def get_by_where(self, where: WhereClause | None = None) -> Model | None:
        """
    ...
    async def get_one_by_where(self, where: WhereClause | None = None) -> Model:
        """
        Get instance from the database that meets the condition.
