...
from domain.schemes.dtos import (
    PasswordCreate,
    PasswordItem,
    PasswordSettings,
    PasswordUpdate,
...
class PasswordService(Service):
    ...
    async def get(self, ident: int) -> PasswordScheme | None:
        ...
        dump["username"] = aes.decrypt(encrypted_scheme.username, settings.infrastructure.encryption.secret)
        dump["password"] = aes.decrypt(encrypted_scheme.password, settings.infrastructure.encryption.secret)

        scheme = PasswordScheme(**dump)

        return scheme

    async def search(self, user: UserScheme, params: Params) -> Page[PasswordItem]:
        async with self.uow:
            return await self.uow.passwords.search(
                params, where=[Password.user_id == user.id], order_by=[Password.id.desc()]
            )
